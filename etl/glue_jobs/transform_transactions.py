...
MCC_CATEGORY_MAP = {
    ...
    "4111": "Transit",       "7011": "Hotels",        "4511": "Airlines",
    "5999": "Retail",        "6011": "ATM",           "5912": "Pharmacy",
    "7372": "Software",      "5045": "Electronics",   "8099": "Healthcare",
}
...
def normalize_currency(df):
    ...
    return df.withColumn(
        "amount_usd",
        F.round(F.col("amount") * exchange_map[F.col("currency")], 2)
    ).withColumn(
...
def deduplicate(df):
    ...
    return df.withColumn("_rank", F.row_number().over(window)).filter(
        F.col("_rank") == 1
    ).drop("_rank")
...
def add_derived_columns(df):
    return df.withColumn(
        ...
        "transaction_hour", F.hour(F.to_timestamp(F.col("transaction_time"), "HH:mm:ss"))
    ).withColumn(
        "is_international", F.col("country_code") != F.lit("US")
    ).withColumn(
        "is_high_value", F.col("amount_usd") > F.lit(10000.0)
    )
